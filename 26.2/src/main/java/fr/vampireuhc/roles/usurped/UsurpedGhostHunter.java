...
/**
 * Chasseur de Fantômes copié par le Sosie : copie exacte du vrai Chasseur avec un petit nerf
 * (fenêtre de 10 minutes au lieu de 20 minutes, gates épisode + jamais deux fois le même joueur,
...
public class UsurpedGhostHunter implements UsurpedPower {
    ...
    public void restoreState(JsonObject obj) {
        ...
        if (obj.has("usurpedGhostHunterTracked") && obj.get("usurpedGhostHunterTracked").isJsonObject()) {
            for (String id : obj.getAsJsonObject("usurpedGhostHunterTracked").keySet()) {
                try {
                    trackedPlayers.add(UUID.fromString(id));
