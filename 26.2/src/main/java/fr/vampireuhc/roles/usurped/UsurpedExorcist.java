...
/**
 * Exorciste usurpé (26.2) : pouvoir imparfait. Le Sosie ne supprime qu'UN SEUL
 * des marqueurs obscurs du joueur, et ne connaît que le NOMBRE de marqueurs
...
public class UsurpedExorcist implements UsurpedPower {
    ...
    public void exorcisePlayer(VampireUHCPlayer target, MarkerManager markerManager, int current_episode) {
        ...
        int count = markerManager.getMarkerTypeOfAura(target.getUuid(), TightAuraTier.OBSCURE).size();
        ...
        bukkitExorcist.sendMessage(net.kyori.adventure.text.minimessage.MiniMessage.miniMessage()
                .deserialize("<dark_purple>Vous avez exorcisé <gold>" + target.getLastKnownName()
                        + "</gold>. Il portait <white>" + count + "</white> marqueur(s) d'aura obscure.</dark_purple>"));
    }
