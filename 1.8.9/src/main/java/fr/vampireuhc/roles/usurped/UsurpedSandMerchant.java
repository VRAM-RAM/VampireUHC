...
public class UsurpedSandMerchant implements UsurpedPower {
    ...
    // Imperfection du Sosie : 1 minute de lenteur au lieu de 3.
    ...
    public void makePlayersSleepOnDeath(MarkerManager manager) {
        ...
        ArrayList<Player> players_with_marker_neutral = manager.getPlayersThatHaveMarkerType(MarkerType.SABLE_NEUTRE_DOPPELGANGER);
        ArrayList<Player> players_with_marker_light = manager.getPlayersThatHaveMarkerType(MarkerType.SABLE_LUMINEUX_DOPPELGANGER);
    ...
    // ligne au moment de la mort du Sosie (consommant ce rôle).
    ...
    private PotionEffect slowness(int amplifier) {
        // 60 secondes d'effet (imperfection : le vrai fait 180).
        return new PotionEffect(PotionEffectType.SLOW, 20 * SLOWNESS_SECONDS, amplifier, true, false);
    }

    private PotionEffect blindness(int amplifier) {
        // 30 secondes d'effet
        return new PotionEffect(PotionEffectType.BLINDNESS, 20 * BLINDNESS_SECONDS, amplifier, true, false);
    }
    ...
    public void restoreState(JsonObject obj) {
        lastSandEpisode = obj.has("usurpedSandEpisode") ? obj.get("usurpedSandEpisode").getAsInt() : -1;
        effectsAppliedAtMillis = obj.has("usurpedSandEffectsAt") ? obj.get("usurpedSandEffectsAt").getAsLong() : -1;
    }
}