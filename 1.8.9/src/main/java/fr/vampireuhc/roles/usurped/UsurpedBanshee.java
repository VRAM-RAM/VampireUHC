...
/**
 * Banshee usurpée : cri exact, mais le message de début d'épisode n'est envoyé
 * qu'au Sosie (pas à tous les joueurs).
 */
public class UsurpedBanshee implements UsurpedPower {
    ...
    private void scream() {
        ...
        if (darkAuras == 0) {
            ...
            bukkitBanshee.sendMessage(MessageUtil.info("La Banshee <gold>pleure</gold>..."));
        } else {
            bukkitBanshee.sendMessage(MessageUtil.info("La Banshee pousse un <gold>cri</gold> déchirant !"));
        }
    }
