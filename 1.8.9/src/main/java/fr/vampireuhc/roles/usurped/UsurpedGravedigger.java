...
/**
 * Fossoyeur usurpé : pouvoir exact (particules sur les cadavres + exhumer la
 * liste des marqueurs du défunt). Seule restriction (asymétrique) : si le vrai
 * Fossoyeur a déjà exhumé le cadavre, le Sosie ne peut pas le ré-exhumer
 * (registre partagé en lecture). Le vrai Fossoyeur, lui, peut exhumer un
 * cadavre déjà exhumé par le Sosie.
 */
public class UsurpedGravedigger implements UsurpedPower {
    ...
    @Override
    public void onExit() {
        // Les cadavres référencent l'ancien monde : on les oublie.
        markersByKey.clear();
    }
    ...
    public void exhum(Location location) {
        ...
        bukkitSosie.sendMessage(MessageUtil.serialize(message));
    }
